...
class PitchMPM
{
  ...
  public:
    ...
    PitchMPM(int sampleRate, size_t bufferSize)
        : bufferSize(bufferSize), sampleRate(sampleRate), fftSize(2 * bufferSize), // Needs to be a power of 2!
          real(audiofft::AudioFFT::ComplexSize(fftSize)), imag(audiofft::AudioFFT::ComplexSize(fftSize)),
          output(fftSize)

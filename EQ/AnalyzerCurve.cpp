...
void AnalyzerCurve::drawNextFrame()
{
        while (mProcessor.mRingBuffer.getNumReady() >= mFFT.getSize())
    {
        ...
        if (block1 > 0) mFFTBuffer.copyFrom (0,      0, mProcessor.mAudioBuffer.getReadPointer (0, start1), block1);
        if (block2 > 0) mFFTBuffer.copyFrom (0, block1, mProcessor.mAudioBuffer.getReadPointer (0, start2), block2);
        ...
        avgFFTBuffer.copyFrom (avgFFTBufferPtr, 0, mFFTBuffer.getReadPointer (0),      avgFFTBuffer.getNumSamples(), 1.0f / (avgFFTBuffer.getNumSamples() * (avgFFTBuffer.getNumChannels() - 1)));
        avgFFTBuffer.addFrom  (0,            0, avgFFTBuffer.getReadPointer (avgFFTBufferPtr), avgFFTBuffer.getNumSamples());

        if (++avgFFTBufferPtr == avgFFTBuffer.getNumChannels()) avgFFTBufferPtr = 1;
    }
}
